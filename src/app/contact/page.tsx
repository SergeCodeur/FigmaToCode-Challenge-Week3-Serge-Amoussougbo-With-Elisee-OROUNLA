...
function Contact() {
  ...
  const ContactItem = ({data}) => {
    return (
      <div className={`flex flex-col transition duration-300  rounded shadow-sm- hover:shadow- py-12 px-6 ${data.bg == "bg-white" ? "bg-white" : 'bg-text_color' }`}>

        <div className={`flex flex-col justify-between flex-grow p-8 borde border-t-0 rounded-b `}>
        <div className="flex justify-center">
        {data?.icon}
        </div>
          <div className={`flex flex-col leading-54 text-sm font-semibold mt-5 text-center gap-4 ${data?.color}`}>
            <div className=""> {data?.c1} </div>
            <div className="">{data?.c2}</div>
            <div className=" mt-4 mx-auto">Get Support</div>
           
          <Button variant="outline" size="lg" rounded="lg" className="border- border-primary text-sm font-medium hover:text-white hover:bg-primary self-center">Submit Request</Button>
          </div>
          {/* <a
            href="/"
...
<Link
  ...
  >
    <i className={`bi ${item.icon}`}></i>
      ...
      <img src="/images/contact-1.svg" className="lg:w-[80%] w-80 object-cover" alt="" />
      </div>
      </div>
      {/* <ContactBanner /> */}
      ...
      <div className="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
        <div className="max-w-xl mb-10 md:mx-auto sm:text-center lg:max-w-2xl md:mb-12">
          ...
          <p className="text-center text-5xl text-text_color md:text-5xl- font-montserrat ">
            We help small businesses <br className="lg:block" /> with big ideas
          </p>
        </div>
